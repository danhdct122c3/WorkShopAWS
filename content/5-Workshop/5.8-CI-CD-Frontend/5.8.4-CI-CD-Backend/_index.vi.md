...
**Điều kiện tiên quyết:** Code Backend đã nằm trên kho lưu trữ GitHub và có chứa file `buildspec-backend.yml`.
...
2. Bấm **Create pipeline**. Chọn **Build custom pipeline** rồi bấm **Next**.
> ![Tạo Pipeline](/aws-image/setupCodePipeline/pipeline2.png)
> ![Chọn Build Custom](/aws-image/setupCodePipeline/pipeline3.png)
3. **Pipeline name**: Đặt tên, ví dụ: `smart-campus-backend-pipeline`. Chọn **New service role** và bấm **Next**.
...
> ![Hoàn tất kết nối 2](/aws-image/setupCodePipeline/pipeline10.png)
6. Trở lại trang cấu hình Source, đảm bảo Repository, nhánh `main` và **CodePipeline default** được chọn. Bấm **Next**.
...
> ![Chọn CodeBuild](/aws-image/setupCodePipeline/pipeline12.png)
2. Trong cửa sổ mới, đặt tên Project là `smart-campus-backend-build`.
> ![Tên CodeBuild](/aws-image/setupCodePipeline/pipeline13.png)
...
> ![Cấu hình Môi trường](/aws-image/setupCodePipeline/pipeline14.png)
4. **Buildspec**: Chọn **Use a buildspec file** và nhập tên file là `buildspec-backend.yml`.
> ![File buildspec](/aws-image/setupCodePipeline/pipeline15.png)
5. Kéo xuống phần Logs, bỏ qua hoặc giữ mặc định rồi bấm **Continue to CodePipeline**.
> ![Tiếp tục](/aws-image/setupCodePipeline/pipeline16.png)
6. Khi có thông báo xanh tạo project thành công, bấm **Next**.
> ![Xác nhận Build Stage](/aws-image/setupCodePipeline/pipeline17.png)
...
> ![Bỏ qua Test](/aws-image/setupCodePipeline/pipeline18.png)
2. Tại bước **Add deploy stage**, bấm **Skip deploy stage**.
> ![Bỏ qua Deploy](/aws-image/setupCodePipeline/pipeline19.png)
3. Màn hình Review hiện ra, cuộn xuống dưới cùng và bấm **Create pipeline**.
> ![Create Pipeline](/aws-image/setupCodePipeline/pipeline20.png)
...
1. Tìm và mở dịch vụ **IAM**.
> ![Tìm IAM](/aws-image/setupCodePipeline/pipeline21.png)
2. Chọn mục **Roles** bên trái, tìm Role có tên chứa `codebuild-smart-campus-backend-build` và bấm vào tên Role đó.
> ![Tìm Role CodeBuild](/aws-image/setupCodePipeline/pipeline22.png)
